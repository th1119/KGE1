...
def r_multi_c(x, r, c, eval_mode=False):
    sqrt_c = c ** 0.5
    xnorm = torch.norm(x, p=1, dim=-1, keepdim=True)
    x_xnorm = torch.sum(x / xnorm, dim=-1, keepdim=True)
    # sqrt_c_x_norm = sqrt_c * xnorm
    num = r * artanh(sqrt_c * xnorm) * x_xnorm
    num = tanh(num) / sqrt_c * x_xnorm
    return num


def m_multi_c(x, m, c, eval_mode=False):
    sqrt_c = c ** 0.5
    xnorm = torch.norm(x, p=1, dim=-1, keepdim=True)
    mx = m * x
    mx_norm = torch.norm(mx, p=1, dim=-1, keepdim=True)
    mx_norm_x_norm = mx_norm / xnorm
    mx_mx_norm = mx / mx_norm
    num = mx_norm_x_norm * artanh(sqrt_c * xnorm)
    num = tanh(mx_mx_norm * num) / sqrt_c * mx_norm_x_norm
    return num