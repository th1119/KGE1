...
class KBCOptimizer(object):
    ...
    def epoch(self, examples: torch.LongTensor, neighbors: torch.LongTensor, position: torch.LongTensor, e=0, weight=None):
        self.model.train()
        ...
        with tqdm.tqdm(total=examples.shape[0], unit='ex', disable=not self.verbose) as bar:
            ...
            while b_begin < examples.shape[0]:
                ...
                predictions, factors = self.model.forward(input_batch, neighbors, position)
                truth = input_batch[:, 2]
