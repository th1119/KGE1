...
class KGOptimizer(object):
    ...
    def new_get_neg_samples(self, input_batch):
        negative_batch = input_batch.repeat(self.neg_sample_size, 1)
        batch_size = input_batch.shape[0]
        ...
        negative_batch[:, 2] = neg_samples
        if self.double_neg:
            neg_samples = torch.Tensor(np.random.randint(
                self.n_entities,
                size=batch_size * self.neg_sample_size)
            ).to(input_batch.dtype)
            negative_batch[:, 0] = neg_samples
        return negative_batch
    ...
    def new_neg_sampling_loss(self, input_batch, neighbors, position):
        positive_score, factors = self.model(input_batch, neighbors, position)
        positive_score = F.logsigmoid(positive_score)

        neg_samples = self.new_get_neg_samples(input_batch)
        negative_score, _ = self.model(neg_samples, neighbors, position)
        negative_score = F.logsigmoid(-negative_score)
        loss = - torch.cat([positive_score, negative_score], dim=0).mean()
        return loss, factors
    ...
    def new_calculate_loss(self, input_batch, neighbors, position):
        if self.neg_sample_size > 0:
            loss, factors = self.new_neg_sampling_loss(input_batch, neighbors, position)
        else:
            predictions, factors = self.model(input_batch, neighbors, position, eval_mode=True)
            truth = input_batch[:, 2]
            loss = self.loss_fn(predictions, truth)
        loss += self.regularizer.forward(factors)
        return loss
    ...
    def new_calculate_valid_loss(self, examples, neighbors, position):
        b_begin = 0
        loss = 0.0
        counter = 0
        with torch.no_grad():
            while b_begin < examples.shape[0]:
                ...
                b_begin += self.batch_size
                loss += self.new_calculate_loss(input_batch, neighbors, position)
                counter += 1
        loss /= counter
        return loss
    ...
    def new_epoch(self, examples, neighbors, position):
        actual_examples = examples[torch.randperm(examples.shape[0]), :]
        with tqdm.tqdm(total=examples.shape[0], unit='ex', disable=not self.verbose) as bar:
            bar.set_description(f'train loss')
            b_begin = 0
            ...
            while b_begin < examples.shape[0]:
                ...
                l = self.new_calculate_loss(input_batch, neighbors, position)
                self.optimizer.zero_grad()
                l.backward()
                self.optimizer.step()
