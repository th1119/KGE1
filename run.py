...
parser.add_argument(
    "--model", default="GIE", choices=all_models, help="Knowledge Graph embedding model"
)
parser.add_argument(
...
def train(args):
    ...
    for step in range(args.max_epochs):
        ...
        model.train()
        train_loss = optimizer.new_epoch(train_examples, dataset.neighbors["train"], dataset.neighbors_addition["train"])
        logging.info("\t Epoch {} | average train loss: {:.4f}".format(step, train_loss))

        model.eval()
        valid_loss = optimizer.new_calculate_valid_loss(valid_examples, dataset.neighbors["valid"], dataset.neighbors_addition["valid"])
        logging.info("\t Epoch {} | average valid loss: {:.4f}".format(step, valid_loss))

        if (step + 1) % args.valid == 0:
            valid_metrics = avg_both(*model.new_compute_metrics(valid_examples, filters, dataset.neighbors["valid"], dataset.neighbors_addition["valid"]))
    ...
    test_metrics = avg_both(*model.new_compute_metrics(test_examples, filters, dataset.neighbors["test"], dataset.neighbors_addition["test"]))
